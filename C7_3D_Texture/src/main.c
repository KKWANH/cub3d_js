...
Uint32				*colorBuffer		= (void *)0;
SDL_Texture			*colorBufferTexture = (void *)0;

Uint32				*wallTexture		= (void *)0;
...
void				setup(void)
{
	...
	for (int x=0; x<TEXTURE_WIDTH; x++)
		for (int y=0; y<TEXTURE_HEIGHT; y++)
			wallTexture[(TEXTURE_WIDTH * y) + x] = (x % 8 == 1 && y % 8 == 1) ? 0xFFFFFF00 : 0xFF000000;
}
...
void				generate3DProjection(void)
{
	...
	for (int i=0; i<NUM_RAYS; i++)
	{
		...
		for (int y=wallBottomPixel; y<WINDOW_HEIGHT; y++)
			colorBuffer[(WINDOW_WIDTH * y) + i] = 0xFF555555;
